...
contract("ChainList", accounts => {
    ...
    it("should sell article", () => ChainList.deployed(instance => {
        chainListInstance = instance;
        return chainListInstance.sellArticle(articleName, articleDescription, web3.toWei(articlePrice, "ether"), {from: seller}).then(() => chainListInstance.getArticle()).then(data => {
    ...
    it('should trigger an event when new article is sold', () => {
        return ChainList.deployed().then((instance) => {
            chainListInstance = instance;
			...
			.then((receipt) => {
				assert.equal(
                    ...
                    "one ebent shouldn't been triggered");
				assert.equal(
                    receipt.logs[0].event, "LogSellArticle",
                    "Event SHould be LogSellArticle");
				assert.equal(
                    receipt.logs[0].args._seller, seller,
                    "event seller must be " + seller);
                assert.equal(
                    receipt.logs[0].args._name, articleName,
                    "event article must be " + articleName);
				assert.equal(
                    receipt.logs[0].args._price.toNumber(),
                    web3.toWei(articlePrice,'ether'),
                    "event seller must be " + articlePrice);
			})
        });
    })
});
